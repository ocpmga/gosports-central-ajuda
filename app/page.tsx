...
export default function HomePage() {
  ...
  // useRef garante que handleSend sempre lê o estado mais recente,
  // sem depender de closure ou useCallback com deps complexas.
  ...
  async function handleSend(question: string) {
    ...
    try {
      // Usa messagesRef.current para histórico sempre atualizado
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 pb-8">
        ...
        <div className="mt-4 flex items-center justify-between px-2">
          <p className="text-xs text-gosports-gray/60">
            Powered by{" "}
            <span className="text-gosports-primary font-semibold">
              GoSports AI
            </span>{" "}
            · Claude Sonnet
          </p>
          ...
          {messages.length > 0 && (
            <button
              ...
              onClick={handleClear}
              disabled={isLoading}
              className="flex items-center gap-1.5 text-xs text-gosports-gray hover:text-gosports-primary disabled:opacity-40 transition-colors font-medium"
            >
              <RefreshCw size={12} />
              Nova conversa
            </button>
