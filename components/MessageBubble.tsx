...
function AvatarAI() {
  return (
    <div className="w-8 h-8 rounded-full bg-gosports-primary flex items-center justify-center flex-shrink-0 shadow-gosports-sm">
...
function Sources({ sources }: { sources: DocumentSource[] }) {
  ...
  return (
    <div className="mt-3 pt-3 border-t border-gosports-border/60 flex flex-wrap gap-1.5">
      <span className="text-xs text-gosports-gray font-medium">Fontes:</span>
      {sources.map((s) => (
        <span
          key={s.id}
          className="text-xs bg-gosports-light text-gosports-secondary border border-gosports-primary/20 px-2 py-0.5 rounded-full font-medium"
        >
          📄 {s.name}
        </span>
      ))}
